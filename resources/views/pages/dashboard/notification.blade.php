<x-dropdown align="right" width="48">
    <x-slot name="trigger">
      <div id="notification-button" @click="visible = !visible" :class="visible ? 'panel-visible border-0 shadow-lg' : 'border panel-hidden'" class="hover:shadow-lg hover:border-opacity-0 hover:bg-gray-50 rounded-full border border-gray-400 h-12 w-12 cursor-pointer font-bold text-2xl text-gray-900 justify-center items-center flex">
          <div class="relative float-right mt-2" v-if="notifications.length > 0">
              <div class="absolute -ml-6 -mt-8">
                  <div class="bg-gray-100 w-8 h-8 rounded-full text-xs flex items-center justify-center">1</div>
              </div>
          </div>
    ...
    <x-slot name="content">
        <div class="absolute bg-white rounded-md left-0 top-0 sm:relative zoom-out-entrance anim-duration-300 h-5/7-screen sm:w-64 sm:h-108 flex flex-row-reverse">
            <div class="z-50 sm:rounded-lg shadow-lg h-full w-full md:mt-2 overflow-y-hidden flex flex-col">
                <div @click="visible = false" class="sm:hidden p-2 cursor-pointer flex items-center justify-center border-b ">
