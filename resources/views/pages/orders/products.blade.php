...
@else
    @foreach($productsDetails as $product)
        <div class="products_item flex" id="content">
                ...
                <div class="flex justify-between product-controls">
                    <div class="-mx-1 flex flex-wrap">
                        <div class="px-1 w-1/2 md:w-auto mb-1">
                            ...
                            <a class="cursor-pointer outline-none border-dashed py-1 border-b-2 border-blue-400 text-sm">
                                {{ __( 'Price' ) }} : <span id="net_purchase_price"> {{ $product["net_purchase_price"] }} fcfa</span>
                            </a>
                        </div>
                        <div class="px-1 w-1/2 md:w-auto mb-1"> 
                            <a onclick="openDiscountPopup()" class="cursor-pointer outline-none border-dashed py-1 border-b-2 border-blue-400 text-sm">{{ __( 'Discount' ) }} <span> 0% </span> :  0 fcfa </a>
                        </div>
                        <div class="px-1 w-1/2 md:w-auto mb-1 lg:hidden"> 
                            <a onclick="changeQuantityNOT()" class="cursor-pointer outline-none border-dashed py-1 border-b-2 border-blue-400 text-sm" id="quantities">{{ __( 'Quantity' ) }}: {{ $product["quantity"] }} </a>
                        </div>
                        <div class="px-1 w-1/2 md:w-auto mb-1 lg:hidden"> 
            ...
            <div onclick="changeQuantityNOT()"  class="hidden lg:flex w-1/6 p-2 border items-center justify-center cursor-pointer">
